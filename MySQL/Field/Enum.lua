...
-- 是否为自增
function Enum:isAutoIncrement()
  return false
end

-- 是否为主键
function Enum:isPrimary()
  return self.primary
end
...
function Enum:toSqlDefine()
  ...
  if type(self.values) == 'string' then
    DDL[#DDL+1] = fmt([[ENUM('%s')]], self.values)
  elseif type(self.values) == 'table' and #self.values > 0 then
    ...
    error("Invalid `Enum` field values.")
  end
